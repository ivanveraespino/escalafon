...
$experienciaArray = [];
$cont=0;
foreach ($experiencia as $puesto) {
$cont++;
$experienciaArray[] = [
'cont'=>$cont,
'tipo'=>$puesto->tipo_entidad,
'entidad' => $puesto->entidad,
'cargo' => $puesto->cargo,
'area'=>$puesto->area,
'inicio' =>  date("Y-m-d", strtotime($puesto->fecha_ini)),
'fin' => date("Y-m-d", strtotime($puesto->fecha_fin)),
'tipodoc' => $puesto->nombredoc,
'nrodoc'=>$puesto->nrodoc,
'archivo'=>$puesto->archivo
];
}

$experienciaJson = json_encode($experienciaArray);
@endphp
<input type="hidden" id="experiencia" name="experiencia" value='{{$experienciaJson}}'>
            ...
            <div class="form-row gap-3">
                <div class="container p-0">
                    <div class="row">
                        ...
                        <div class="col-12 col-sm-6">
                            <div class="form-floating">
                                <input type="text" class="form-control" id="area-exp" name="area-exp">
                                <label for="area-exp">Área<span class="required">*</span></label>
                            </div>
                        </div>
                        <div class="col-12 col-sm-6">
                            <div class="form-floating ">
        ...
        <div class="col-12 col-sm-6">
            <div id="tabla-experiencia">
                <table class="table">
                    ...
                    <tbody>
                        @foreach($experienciaArray as $puesto)
                        <tr>
                            <td>{{ $puesto['cargo'] }}</td>
                            <td>{{ $puesto['entidad'] }}</td>
                            <td>{{ \Carbon\Carbon::parse($puesto['inicio'])->diff(\Carbon\Carbon::parse($puesto['fin']))->format('%y A, %m M, %d D') }}</td>
                            <td>
                                <a class="btn btn-outline-info btn-sm" onClick="editarExperiencia({{$puesto['cont']}})">
                                    <i class="fa fa-edit"></i>
                                </a>
                                <a class="btn btn-outline-danger btn-sm" onClick="anularExperiencia({{$puesto['cont']}})">
